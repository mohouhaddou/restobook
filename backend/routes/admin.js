...
const DEFAULT_HERO   = process.env.DEFAULT_HERO_URL || '/img/hero.jpg';
...
function absolutizeMaybe(u, req) {
  ...
  if (!s.startsWith('/')) s = '/' + s;
  if (s.startsWith('/brand/')) s = `/restobook${s}`;
...
function buildSettingsResponse(dict, req) {
  ...
  return { cutoff_time, allow_cancel_until, hero_image_url, brand_name, brand_logo_url, theme_primary, theme_accent };
}
...
router.put('/settings', requirePermission(PERMISSIONS.SETTINGS_MANAGE), ah(async (req, res) => {
  const { cutoff_time, allow_cancel_until, hero_image_url, brand_name, brand_logo_url, theme_primary, theme_accent } = req.body || {};
  const orgId = req.user.organization_id;
  ...
  if (brand_name !== undefined) await upsertSetting('brand_name', cleanNullable(brand_name) || 'RestoBook', orgId);
  ...
  if (theme_primary !== undefined) {
    if (!isHexColor(theme_primary)) return res.status(400).json({ error: 'theme_primary invalide (#RRGGBB)' });
    await upsertSetting('theme_primary', theme_primary.toUpperCase(), orgId);
  }
  if (theme_accent !== undefined) {
    if (!isHexColor(theme_accent)) return res.status(400).json({ error: 'theme_accent invalide (#RRGGBB)' });
    await upsertSetting('theme_accent', theme_accent.toUpperCase(), orgId);
  }
