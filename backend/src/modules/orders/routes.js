...
router.patch('/:id/status',
  ...
  [
    ...
    body('status').isIn(['confirmed','preparing','ready','picked_up','on_the_way','delivered','cancelled']).withMessage('Statut invalide'),
  ],
  validate,
  async (req, res, next) => {
    try {
      ...
      // Fidélité — points + cashback crédités à la remise effective (idempotent),
      // réversion si une commande déjà remise est ensuite annulée.
      const { onOrderDelivered, onOrderCancelledAfterDelivery } = require('../loyalty/orderHooks');
      if (req.body.status === 'delivered') {
        onOrderDelivered(order).catch(() => {});
      } else if (req.body.status === 'cancelled' && prevStatus === 'delivered') {
        onOrderCancelledAfterDelivery(order, req.user.id).catch(() => {});
      }

      // Dispatch automatique — dès que la commande est prête, propose la
      // livraison au meilleur livreur disponible si le commerce a activé un
      // mode de livraison (no-op silencieux sinon : delivery_mode='disabled'
      // par défaut pour tous les commerces existants, comportement inchangé).
      if (req.body.status === 'ready' && order.type === 'delivery') {
        require('../delivery/services/dispatchEngine').dispatchOrder(order).catch(() => {});
      }
