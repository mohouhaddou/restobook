...
router.patch('/profile',
  ...
  [
    ...
    body('accepts_reservation').optional().isBoolean(),
    body('accepts_qr_table').optional().isBoolean(),
    body('opening_hours').optional().isObject(),
    body('latitude').optional().isFloat({ min: -90, max: 90 }),
    body('longitude').optional().isFloat({ min: -180, max: 180 }),
    body('location_verified').optional().isBoolean(),
    body('cover_url').optional().trim().isLength({ max: 500 }),
...
router.get('/tables/:id/qr', async (req, res, next) => {
  try {
    ...
    const basePath = process.env.APP_BASE_PATH || '/restobook';
    const qrUrl   = `${baseUrl}${basePath}/#/qr/${org.slug}/${table.qr_token}`;
