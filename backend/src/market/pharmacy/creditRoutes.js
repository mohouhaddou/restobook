...
/**
 * Module Crédit Clients — gestion des ventes à crédit (دين/كريدي) de la pharmacie.
 * Toutes les routes sont montées sous /api/pharmacy-pro/credit
 */
...
const {
  sequelize, PharmacyCustomer, PharmacyCredit, PharmacyCreditPayment, PharmacyCreditAuditLog,
} = require('../../../models');
const { requireAuth, requirePermission } = require('../../../middleware/auth');
const { PERMISSIONS } = require('../../../auth/permissions');
const validate = require('../../../middleware/validate');
const { recomputeLedger: recomputeLedgerShared } = require('../../utils/pharmacyCreditLedger');

const ah = fn_ => (req, res, next) => Promise.resolve(fn_(req, res, next)).catch(next);

router.use(requireAuth, requirePermission(PERMISSIONS.PHARMACY_CREDIT_VIEW));
...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename:    (req, file, cb) => cb(null, `pharmacredit_${Date.now()}_${file.originalname.replace(/[^a-zA-Z0-9._-]/g, '_')}`),
  }),
  limits: { fileSize: 5 * 1024 * 1024 },
  fileFilter: (req, file, cb) => cb(null, /^image\/(jpeg|png|webp|gif)$/.test(file.mimetype)),
});

router.post('/upload', requirePermission(PERMISSIONS.PHARMACY_CREDIT_MANAGE), upload.single('image'), ah(async (req, res) => {
...
async function overdueCountMap(customerIds, t) {
  ...
  const rows = await PharmacyCredit.findAll({
...
function recomputeLedger(customerId, organizationId, t) {
  return recomputeLedgerShared({ PharmacyCredit, PharmacyCreditPayment, PharmacyCustomer }, customerId, organizationId, t);
}

async function logAudit(req, action, entity_id, details) {
  try {
    await PharmacyCreditAuditLog.create({
...
router.get('/customers', [
  ...
  const rows = await PharmacyCustomer.findAll({ where, order: [['balance', 'DESC']] });
...
router.post('/customers', requirePermission(PERMISSIONS.PHARMACY_CREDIT_MANAGE), [
  ...
  const oid = orgId(req);
  const customer = await PharmacyCustomer.create({
    organization_id: oid,
...
router.get('/customers/:id', [param('id').isInt()], validate, ah(async (req, res) => {
  ...
  const [credits, payments] = await Promise.all([
    PharmacyCredit.findAll({ where: { customer_id: customer.id }, order: [['date', 'DESC'], ['id', 'DESC']] }),
    PharmacyCreditPayment.findAll({ where: { customer_id: customer.id }, order: [['date', 'DESC'], ['id', 'DESC']] }),
  ]);

  const overdue_count = await PharmacyCredit.count({ where: { customer_id: customer.id, due_date: { [Op.lt]: today() }, status: { [Op.ne]: 'paid' } } });
...
// PATCH /customers/:id
router.patch('/customers/:id', requirePermission(PERMISSIONS.PHARMACY_CREDIT_MANAGE), [
  param('id').isInt(),
...
router.delete('/customers/:id', requirePermission(PERMISSIONS.PHARMACY_CREDIT_DELETE), ah(async (req, res) => {
  const oid = orgId(req);
  const customer = await PharmacyCustomer.findOne({ where: { id: req.params.id, organization_id: oid } });
  if (!customer) return res.status(404).json({ error: 'Client introuvable' });
...
router.post('/credits', requirePermission(PERMISSIONS.PHARMACY_CREDIT_MANAGE), [
  ...
  const oid = orgId(req);
  const customer = await PharmacyCustomer.findOne({ where: { id: req.body.customer_id, organization_id: oid } });
  if (!customer) return res.status(404).json({ error: 'Client introuvable' });

  const t = await sequelize.transaction();
  try {
    const credit = await PharmacyCredit.create({
...
router.get('/credits', [
  ...
  const page  = Math.max(1, Number(req.query.page || 1));
  const limit = Math.min(200, Number(req.query.limit || 50));
  const { count, rows } = await PharmacyCredit.findAndCountAll({
    where,
    include: [{ model: PharmacyCustomer, as: 'customer', attributes: ['id', 'name', 'phone'] }],
    order: [['date', 'DESC'], ['id', 'DESC']], limit, offset: (page - 1) * limit,
  });
  res.json({ total: count, page, pages: Math.ceil(count / limit) || 1, credits: rows });
}));

router.delete('/credits/:id', requirePermission(PERMISSIONS.PHARMACY_CREDIT_DELETE), ah(async (req, res) => {
  const oid = orgId(req);
  const credit = await PharmacyCredit.findOne({ where: { id: req.params.id, organization_id: oid } });
...
router.post('/payments', requirePermission(PERMISSIONS.PHARMACY_CREDIT_PAYMENT_MANAGE), [
  body('customer_id').isInt({ min: 1 }),
  body('amount').isFloat({ gt: 0 }),
  ...
  const t = await sequelize.transaction();
  try {
    const payment = await PharmacyCreditPayment.create({
      organization_id: oid, customer_id: customer.id,
...
router.get('/payments', [
  ...
  const page  = Math.max(1, Number(req.query.page || 1));
  const limit = Math.min(200, Number(req.query.limit || 50));
  const { count, rows } = await PharmacyCreditPayment.findAndCountAll({
    where,
    include: [{ model: PharmacyCustomer, as: 'customer', attributes: ['id', 'name', 'phone'] }],
    order: [['date', 'DESC'], ['id', 'DESC']], limit, offset: (page - 1) * limit,
  });
...
router.delete('/payments/:id', requirePermission(PERMISSIONS.PHARMACY_CREDIT_DELETE), ah(async (req, res) => {
  const oid = orgId(req);
  const payment = await PharmacyCreditPayment.findOne({ where: { id: req.params.id, organization_id: oid } });
...
router.get('/dashboard', ah(async (req, res) => {
  ...
  const [totalCreditsAgg, totalPaymentsAgg, monthPaymentsAgg, todayPaymentsAgg, customers, overdueTotal] = await Promise.all([
    PharmacyCredit.sum('amount', { where: { organization_id: oid } }),
    PharmacyCreditPayment.sum('amount', { where: { organization_id: oid } }),
    PharmacyCreditPayment.sum('amount', { where: { organization_id: oid, date: { [Op.gte]: monthStart } } }),
    PharmacyCreditPayment.sum('amount', { where: { organization_id: oid, date: todayStr } }),
    PharmacyCustomer.findAll({ where: { organization_id: oid, active: true } }),
    PharmacyCredit.count({ where: { organization_id: oid, due_date: { [Op.lt]: todayStr }, status: { [Op.ne]: 'paid' } } }),
  ]);
  ...
  const [creditRows, paymentRows] = await Promise.all([
    PharmacyCredit.findAll({ where: { organization_id: oid, date: { [Op.gte]: rangeStart } }, attributes: ['date', 'amount'], raw: true }),
    PharmacyCreditPayment.findAll({ where: { organization_id: oid, date: { [Op.gte]: rangeStart } }, attributes: ['date', 'amount'], raw: true }),
  ]);
...
router.get('/history', [
  ...
  const [credits, payments] = await Promise.all([
    PharmacyCredit.findAll({ where: { organization_id: oid }, include: [{ model: PharmacyCustomer, as: 'customer', attributes: ['id', 'name'] }], order: [['date', 'DESC']], limit: 300 }),
    PharmacyCreditPayment.findAll({ where: { organization_id: oid }, include: [{ model: PharmacyCustomer, as: 'customer', attributes: ['id', 'name'] }], order: [['date', 'DESC']], limit: 300 }),
  ]);
...
router.get('/audit-log', [query('page').optional().isInt({ min: 1 })], validate, ah(async (req, res) => {
  ...
  const page  = Math.max(1, Number(req.query.page || 1));
  const limit = 50;
  const { count, rows } = await PharmacyCreditAuditLog.findAndCountAll({
